...
namespace one2many_counter_queue_impl
{
...
public:
    // ctor
    one2many_counter_guard(one2many_counter_bucket<event_t>& bucket, std::uint64_t owner) : m_bucket(&bucket), m_owner(owner)
    {
    }
    ...
    {
        if (m_owner != one2many_counter_queue_impl::DUMMY_READER_ID)
        {
            auto const before = m_bucket->m_counter.fetch_add(-1, std::memory_order_release);
            auto const release_etalon(one2many_counter_queue_impl::CONSTRUCTED_DATA_MARK + 1);

            if (before == release_etalon)
            {
                m_bucket->get_event().~event_t();
                m_bucket->m_counter.store(one2many_counter_queue_impl::EMPTY_DATA_MARK, std::memory_order_release);
            }
        }
    }
...
private:
    one2many_counter_bucket<event_t>* m_bucket;
    std::uint64_t m_owner;
};

// bucket
template<class event_t>
struct alignas(one2many_counter_queue_impl::CPU_CACHE_LINE_SIZE) one2many_counter_bucket
{
    using storage_t = typename std::aligned_storage<sizeof(event_t), alignof(event_t)>::type;

    one2many_counter_bucket() :
        m_seqn(one2many_counter_queue_impl::DUMMY_EVENT_SEQ_NUM),
        m_counter(one2many_counter_queue_impl::EMPTY_DATA_MARK)
    {
    }
    ...
    ~one2many_counter_bucket()
    {
        if (m_counter != one2many_counter_queue_impl::EMPTY_DATA_MARK)
        {
            get_event().~event_t();
            m_counter.store(one2many_counter_queue_impl::EMPTY_DATA_MARK, std::memory_order_relaxed);
        }
...
static_assert(sizeof(one2many_counter_bucket<void*>) == one2many_counter_queue_impl::CPU_CACHE_LINE_SIZE, "Performance optimization");

// reader
template<class event_t>
class alignas(one2many_counter_queue_impl::CPU_CACHE_LINE_SIZE) one2many_counter_reader
{
public:
    // ctor
    one2many_counter_reader(std::shared_ptr<one2many_counter_queue<event_t>> queue, one2many_counter_bucket<event_t>* storage, std::uint64_t storage_size, std::uint64_t read_from, std::uint64_t id) :
        m_next_read_index(read_from), m_storage_size(storage_size), m_storage(storage), m_id(id), m_queue(std::move(queue))
    {
    ...
    // move ctor and assign
    one2many_counter_reader(one2many_counter_reader&&) = delete;
    one2many_counter_reader& operator=(one2many_counter_reader&&) = delete;
    ...
    std::pair<bool, one2many_counter_guard<event_t>> try_read() noexcept
    {
        bool result = false;
        ...
        return std::make_pair(result, one2many_counter_guard<event_t>(bucket, result ? m_id : one2many_counter_queue_impl::DUMMY_READER_ID));
    }

    one2many_counter_guard<event_t> read() noexcept
    {
        read_mark:
...
private:
    ...
    one2many_counter_bucket<event_t>* const m_storage;
    std::uint64_t const m_id;
    std::shared_ptr<one2many_counter_queue<event_t>> const m_queue;
};

static_assert(sizeof(one2many_counter_reader<void*>) == one2many_counter_queue_impl::CPU_CACHE_LINE_SIZE, "Performance optimization");

// queue
template<class event_t>
class alignas(one2many_counter_queue_impl::CPU_CACHE_LINE_SIZE) one2many_counter_queue : public std::enable_shared_from_this<one2many_counter_queue<event_t>>
{
public:
    using queue_ptr = std::shared_ptr<one2many_counter_queue<event_t>>;
    using reader_ptr = std::unique_ptr<one2many_counter_reader<event_t>>;
...
public:
    ...
    one2many_counter_queue(one2many_counter_queue&&) = delete;
    one2many_counter_queue& operator=(one2many_counter_queue&&) = delete;
    ...
    // allocate new queue.
    static std::shared_ptr<one2many_counter_queue<event_t>> make(std::size_t n)
    {
        struct make_shared_enabler : public one2many_counter_queue<event_t>
        {
            make_shared_enabler(std::size_t n) : one2many_counter_queue<event_t>(n)
            {
            }
        };
    ...
    std::unique_ptr<one2many_counter_reader<event_t>> create_reader()
    {
        ...
        if (next_id == one2many_counter_queue_impl::DUMMY_READER_ID)
        {
            throw std::runtime_error("Next reader id overflow: " + std::to_string(next_id) + " > " + std::to_string(one2many_counter_queue_impl::DUMMY_READER_ID));
        }
    ...
    bool try_write(event_t&& event) noexcept
    {
        ...
        if (bucket.m_counter.load(std::memory_order_acquire) == one2many_counter_queue_impl::EMPTY_DATA_MARK)
        {
            auto const num_readers = m_local.m_next_reader_id.load(std::memory_order_acquire);

            new (&bucket.m_storage) event_t(std::move(event));
            bucket.m_counter.store(num_readers + one2many_counter_queue_impl::CONSTRUCTED_DATA_MARK, std::memory_order_release);
    ...
    std::uint64_t get_alive_mask() const noexcept
    {
        auto const num_readers = m_local.m_next_reader_id.load(std::memory_order_acquire);
        std::uint64_t mask{0};
        for (std::uint64_t i = 0; i < num_readers; i++)
        {
...
private:
    ...
    struct alignas(one2many_counter_queue_impl::CPU_CACHE_LINE_SIZE)local_t
    {
        using storage_t = std::unique_ptr<one2many_counter_bucket<event_t>[]>;

        local_t(std::size_t n) :
            m_next_seq_num(one2many_counter_queue_impl::MIN_EVENT_SEQ_NUM),
            m_storage_size(n),
            m_storage(new typename storage_t::element_type[n]),
            m_next_reader_id(one2many_counter_queue_impl::MIN_READER_ID)
        {
        }
...
static_assert(sizeof(one2many_counter_queue<void*>) == one2many_counter_queue_impl::CPU_CACHE_LINE_SIZE * 2, "Performance optimization");
